...
from app.core.scoring.activity import (calculate_raw_activity_score, calucate_activity_score)
...
def test_calucate_activity_score():
    metrics = RepoActivityMetrics(
        commits_last_30_days=30,
        issues_closed_last_30_days=20,
        prs_merged_last_30_days=10
    )
    score = calucate_activity_score(metrics)
    assert 0 <= score <= 100